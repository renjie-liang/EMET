...
class MultiTeacherEnsemble:
    ...
    def _build_model(self, word_vectors):
        ...
        ### --------------- Ensemble Branch ------------------
        # vfeats # [B, 64, 128]
        w0 = conv1d_bn_relu(vfeats, dim=16, kernel_size=3, reuse=False, name='ens_conv0') # [B, 64, 16]
        w1 = conv1d_bn_relu(vfeats, dim=16, kernel_size=5, reuse=False, name='ens_conv1') # [B, 64, 16]
        w2 = conv1d_bn_relu(vfeats, dim=16, kernel_size=7, reuse=False, name='ens_conv2') # [B, 64, 16]
        

        ## element wise multiply
        w3 = tf.reduce_max(qfeats, axis=1)# [B, 128]
        w3 = tf.expand_dims(w3, axis=1)  # (B, 1, 128)
        w3 = conv1d_bn_relu(w3, dim=16, kernel_size=1, reuse=False, name='ens_conv3') # [B, 1, 16]
       
        ensemble_feat = tf.stack([w0, w1, w2], axis=1) # (B, 3, 64, 16) 
        ensemble_feat = tf.reduce_sum(ensemble_feat, axis=1) * w3  # [B, 64, 16]
        ensemble_feat = tf.reduce_mean(ensemble_feat, axis=2, keepdims=False)  # [B, 64]

        B = get_shape_list(ensemble_feat)[0]
        ensemble_feat = tf.reshape(ensemble_feat, [B, self.configs.model.max_vlen])
        ...
        softloss_label = label_kdfunc(self.slogits, label_T[:, 0, :], self.configs.loss.label_temperature, v_mask) \
                 + label_kdfunc(self.elogits, label_T[:, 1, :], self.configs.loss.label_temperature, v_mask)
        softloss_label = self.configs.loss.label_cof * softloss_label
        
        # self.tmp = ensemble_weight
        # hard loss for ensemble label
        hardloss_ensemble = localizing_loss(label_T[:, 0, :], label_T[:, 1, :], self.y1, self.y2, v_mask)
        # l1_regularization = 0.01 * tf.reduce_sum(tf.abs(ensemble_weight))
        # l2_regularization = 0.01 * tf.reduce_sum(tf.square(ensemble_weight))
        # hardloss_ensemble = hardloss_ensemble  + l2_regularization + l1_regularization
        
        # soft loss for internal feature
        # inter_loss_0 = inter_kdfunc(outputs, t0_outputs, self.configs.loss.inter_temperature, tf.expand_dims(v_mask, axis=-1))
        # inter_loss_1 = inter_kdfunc(fuse_feats, t0_fuse_feats, self.configs.loss.inter_temperature, tf.expand_dims(v_mask, axis=-1))
        # softloss_internal = self.configs.loss.inter_cof_0 * inter_loss_0 + self.configs.loss.inter_cof_1 * inter_loss_1
        # total loss
        self.loss = hardloss_s + hardloss_t0  + hardloss_ensemble + softloss_label#  + softloss_internal
